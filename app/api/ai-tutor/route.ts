...
async function getTutorContext(): Promise<{ context: TutorContext; userId: string } | null> {
  ...
  const [{ data: profile }, { data: progress }, { data: placement }] = await Promise.all([
    ...
    supabase.from('learning_section_progress').select('level_code,lesson_number,section,status,updated_at').eq('user_id', user.id).order('updated_at', { ascending: false, nullsFirst: false }).limit(20),
  ...
  const recentLessons = Array.from(new Set((progress ?? []).map((item: any) => `${String(item.level_code || 'A1').toUpperCase()}، الدرس ${Number(item.lesson_number || 1)}: ${String(item.section || 'قسم')} (${item.status === 'completed' ? 'مكتمل' : 'قيد التعلّم'})`))).slice(0, 8);
  const rawInterests = profile?.interests ?? user.user_metadata?.interests;
  const interests = Array.isArray(rawInterests) ? rawInterests.map(String).filter(Boolean) : typeof rawInterests === 'string' ? rawInterests.split(/[،,]/u).map((value: string) => value.trim()).filter(Boolean) : [];
  return { userId: user.id, context: { name: profile?.full_name || user.user_metadata?.full_name || 'الطالب', level: placement?.recommended_level || user.user_metadata?.level || 'A1', track: profile?.arabic_track || user.user_metadata?.arabic_track || 'الفصحى', goal: profile?.learning_goal || user.user_metadata?.learning_goal || 'التحدث بالعربية بثقة', interests, recentLessons, completedCount: (progress ?? []).filter((item: any) => item.status === 'completed').length } };
}
