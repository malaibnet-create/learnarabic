...
export async function POST(request: Request) {
  try {
    ...
    if (configured) {
      ...
      userId = user.id;
      const { data } = await supabase.from('level3_exam_attempts').select('*').eq('id', body.attemptId).eq('user_id', user.id).maybeSingle();
      attempt = data;
    ...
    try {
      ...
      if (supabase && attempt) await supabase.from('level3_exam_attempts').update({ status: 'evaluation_pending', answers, writing_text: writingText, speaking_transcript: speakingTranscript, updated_at: new Date().toISOString() }).eq('id', attempt.id).eq('user_id', userId);
